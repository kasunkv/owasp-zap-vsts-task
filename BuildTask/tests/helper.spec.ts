...
describe('OWASP Zap Scan Helpers', function() {
    ...
    describe('When a valid xmlReport with two site elements and a url is passed, the return value', () => {
        let helper: Helper;
    ...
    describe('When a valid xmlReport with one site elements and a url is passed, the return value', () => {
        let helper: Helper;
        let xmlString: string;
        let result: AlertResult;
        const validTargetUrl: string = 'http://k2vowasptestsite.azurewebsites.net';
        ...
        before(() => {
            helper = new Helper();
            let xmlPath = path.join(__dirname, 'valid-one-site.xml');
            xmlString = fs.readFileSync(xmlPath, 'utf8');
            result = helper.ProcessAlerts(xmlString, validTargetUrl);
        });
